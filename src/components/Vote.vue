<template>
  <div id="vote">
    ...
    <div class="overlay">
      ...
      <em class="blue">{{ nominator.name }}</em> 提名
      <em>{{ nominee.name }}</em
      >!
      <br />
      <em class="blue">
      ...
      <div v-else-if="!player">请在座位上才能投票。</div>
    </div>
...
export default {
  computed: {
    ...
    player: function () {
      return this.players.find((p) => p.id === this.loginbackend.playerId);
    },
    currentVote: function () {
      const index = this.players.findIndex(
        (p) => p.id === this.loginbackend.playerId,
      );
